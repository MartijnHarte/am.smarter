'use strict';
//TODO check socket.all_sorts_of_data and implement it in driver
//TODO continue implementing iKettles
...
var iKettles = function () {

}

iKettles.prototype = new EventEmitter()


iKettles.prototype.discover = function(callback) {
    var discoverStartTime = new Date().getTime()
    var self = this;

    var device_emitter = new EventEmitter();
    var final_emitter = new EventEmitter();

    discover( device_emitter )
    device_emitter.on('done', function(){
		final_emitter.emit("done");
    })
    device_emitter.on( 'found_device', function ( socket ) {
        self._discovered( socket, discoverStartTime, final_emitter )
    } )
    if(callback) callback(final_emitter);

}

iKettles.prototype._discovered = function( socket, discoverStartTime, final_emitter) {
    var kettle = new iKettle()
    kettle.kettle = socket;

    kettle.onoff= false;
    kettle.keep_warm=false;
    kettle.keep_warm_time=null;
    kettle.keep_warm_expired=false;
    kettle.boiled=false;
    kettle.temperature=null;

    kettle._setupListeners()
    ...
    if(final_emitter) final_emitter.emit('found_device', kettle);
}
    ...
    if (code[2] && code[2].match(/[0-9]*x[0-9]*/)) {
        switch (code[2]) {
          case '0x0':
              ...
              this.onoff = false;
              this.boiled = false;
              this.keep_warm = false;
              this.keep_warm_expired = false;
              break
          case '0x1':
              this.emit('removed')
              break
          ...
          case '0x8005':
              ...
              this.keep_warm_time = 5 * 60 * 1000;
              break
          case '0x8010':
              this.emit('keep-warm-time', 10 * 60 * 1000)
              this.keep_warm_time = 10 * 60 * 1000;
              break
          case '0x8020':
              this.emit('keep-warm-time', 20 * 60 * 1000)
              this.keep_warm_time = 20 * 60 * 1000;
              break
        }
    }
}
